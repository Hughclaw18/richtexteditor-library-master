...
var wyswygOptions = function(json) {
    return {
        ...
        features: ["list", 'code_block', 'blockquote', 'hr', slashCommandFeature, 'emoji', 'tables'],
        menubar: {position: 'top', overrides: {fontSize: {addMenu: () => {}}, inlineQuote: {addMenu: () => {}}}, menuItemOverrides: {blockquote: {icon: 'zmetbi-quote'}}},
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelector('#rte-mode').addEventListener('click', ()=> {
        if (zwRteView.constructor.name === 'MarkdownView') {
            var md = zwRteView.getText()
            window.zwRteView.remove()
            var json = RichTextEditor.md2doc(md)
            window.zwRteView = RichTextEditor.init(document.getElementById('rte-wrapper-container'), wyswygOptions(json));
        }
    })

    document.querySelector('#md-mode').addEventListener('click', ()=> {
        if (zwRteView.constructor.name === 'RichTextEditorView') {
            var json = zwRteView.getJSON()
            window.zwRteView.remove()
            window.zwRteView = RichTextEditor.init(document.getElementById('rte-wrapper-container'), {
                mode: 'markdown',
                content: RichTextEditor.doc2md(json),
                placeholder: 'Type something...'

                // onEditorStateUpdate: (s, v) => { console.log(s, v) }
            });
        }
    })
})
...
function getInputRules() {
    ...
    var underlineInputRule = markInputRule(/(?:^|\s)(\_\_)([^*]+)(\_\_)$/g, state => state.schema.marks.underline)
    var strikethroughInputRule = markInputRule(/(?:^|\s)(\~\~)([^*]+)(\~\~)$/g, state => state.schema.marks.strikeThrough)
    ...
    return [boldInputRule, inlineQuoteInputRule, headingsInputRule, quoteInputRule, hrInputRule, underlineInputRule, italicsInputRule, linkInputRule, strikethroughInputRule]
}
